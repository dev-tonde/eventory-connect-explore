...
import { Calendar, Users, DollarSign, TrendingUp, Eye, Share, Heart } from "lucide-react";
...
const AdvancedEventAnalytics = ({ eventId }: { eventId?: string }) => {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      const analyticsPromises = events.map(async (event) => {
        ...
        const totalRevenue = tickets?.reduce((sum, t) => sum + Number(t.total_price), 0) || 0;
        ...
        return {
          ...
          conversionRate: views?.length ? (totalTickets / views.length) * 100 : 0,
          dailyViews,
          sourceBreakdown: [
  ...
  const aggregatedData = analytics.reduce((acc, event) => ({
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{aggregatedData.totalViews.toLocaleString()}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{aggregatedData.totalFavorites}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{aggregatedData.totalShares}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{aggregatedData.totalTickets}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">R{aggregatedData.totalRevenue.toLocaleString()}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={analytics[0]?.dailyViews || []}>
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      data={analytics[0]?.sourceBreakdown || []}
                      cx="50%"
                      cy="50%"
                      labelLine={false}
                      ...
                      dataKey="count"
                    >
                      {analytics[0]?.sourceBreakdown.map((entry, index) => (
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {analytics.map(event => (
                  <div key={event.eventId} className="flex justify-between items-center p-4 border rounded">
                    <div>
                      <h3 className="font-medium">{event.eventTitle}</h3>
                      <p className="text-sm text-gray-600">{event.views} views → {event.tickets} tickets</p>
                    </div>
        ...
        <TabsContent value="demographics">
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={analytics[0]?.demographicData || []}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="age" />
                  <YAxis />
                  <Tooltip />
