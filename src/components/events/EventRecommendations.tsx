  ...
  const { data: recommendations = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const scoredEvents = data.map(event => {
        ...
        // Category matching
        if (userPreferences?.pastEvents?.some((pe: any) => 
          pe.events?.category === event.category
        )) {
          score += 20;
        ...
        const avgUserPrice = userPreferences?.pastEvents?.reduce((sum: number, pe: any) => 
          sum + (pe.events?.price || 0), 0
        ) / (userPreferences?.pastEvents?.length || 1);
        ...
        // Tag matching
        const userTags = [
          ...userPreferences?.pastEvents?.flatMap((pe: any) => pe.events?.tags || []) || [],
          ...userPreferences?.favorites?.flatMap((f: any) => f.events?.tags || []) || []
        ];
          ...
          <Card key={event.id} className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-1">
                <div className="text-xs font-medium text-purple-600 mb-1">Why this matches:</div>
                {event.matchReasons?.slice(0, 2).map((reason: string, index: number) => (
