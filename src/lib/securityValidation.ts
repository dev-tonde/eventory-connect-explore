...
export const validateSecureUrl = (
  url: string
): { isValid: boolean; error?: string } => {
  try {
    ...
    if (
      ...
      blockedHosts.some((blocked) => hostname.startsWith(blocked))
    ) {
...
// Rate limiting validation (client-side, sessionStorage)
...
// Content Security Policy header validation/generation
...
// This CSP header allows scripts and styles from trusted sources, blocks inline scripts unless they have a valid nonce, and restricts connections to the specified domains.
