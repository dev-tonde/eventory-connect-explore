...
import { calculateAppliedRules, calculateNextPriceChange, getMockPriceHistory } from "@/utils/pricingCalculations";
...
export const useDynamicPricing = (eventId: string, basePrice: number) => {
  ...
  const { pricingRules, updatePricingRule, isUpdatingRule } = usePricingRules(eventId);
  ...
  const { data: dynamicPriceData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        nextPriceChange: calculateNextPriceChange(appliedRules, eventData, basePrice)
      } as DynamicPriceData;
    },
