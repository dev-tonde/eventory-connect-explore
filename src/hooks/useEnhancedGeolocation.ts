...
export const useEnhancedGeolocation = (): GeolocationHook => {
  ...
  const getCurrentLocation = useCallback(async (): Promise<LocationData | null> => {
    ...
    try {
      ...
      const errorMessage = "Unable to get your location. Please check permissions.";
  ...
  const geocodeAddress = useCallback(async (address: string): Promise<LocationData | null> => {
    ...
    try {
      ...
      const { data: cachedLocation } = await supabase
        .rpc('get_cached_location', { _address: address });
      ...
      // Use browser's geocoding or mock for now
...
const mockGeocodeService = async (address: string): Promise<LocationData | null> => {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 500));
  
  // Mock coordinates for common cities
  const mockLocations: Record<string, LocationData> = {
    "New York": { latitude: 40.7128, longitude: -74.0060, address, city: "New York", country: "USA" },
    "London": { latitude: 51.5074, longitude: -0.1278, address, city: "London", country: "UK" },
    "Paris": { latitude: 48.8566, longitude: 2.3522, address, city: "Paris", country: "France" },
    "Tokyo": { latitude: 35.6762, longitude: 139.6503, address, city: "Tokyo", country: "Japan" },
  };
  ...
  return found ? mockLocations[found] : {
...
const mockReverseGeocodeService = async (lat: number, lng: number): Promise<LocationData> => {
