...
interface SecurityEvent {
  id: string;
  type: 'login_attempt' | 'payment_attempt' | 'data_access' | 'suspicious_activity';
...
export const useSecurityAudit = () => {
  ...
  const logSecurityEvent = async (event: Omit<SecurityEvent, 'id' | 'timestamp'>) => {
    try {
      ...
      // Add to local state for monitoring
  ...
  const detectSuspiciousActivity = () => {
    // Track rapid successive actions
    let actionCount = 0;
  ...
  const trackPageAccess = (page: string) => {
    ...
    const sensitivePaths = ['/dashboard', '/admin', '/create-event', '/profile'];
  ...
  const trackPaymentAttempt = (amount: number, eventId: string, success: boolean) => {
    logSecurityEvent({
      type: 'payment_attempt',
      severity: success ? 'low' : 'medium',
      message: `Payment ${success ? 'succeeded' : 'failed'} for R${amount}`,
      metadata: { amount, eventId, success, timestamp: new Date().toISOString() }
    });
  };
