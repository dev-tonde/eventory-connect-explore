...
const corsHeaders = {
  "Access-Control-Allow-Origin": "https://staging.eventory.co.za", // Restrict to your trusted domain
  Vary: "Origin",
...
const sanitizeMetadata = (
  ...
  if (!metadata || typeof metadata !== "object") {
    return { eventId: "", userId: "", quantity: "1" };
  }
  return {
    eventId:
      typeof metadata.eventId === "string"
        ? metadata.eventId.slice(0, 100)
        : "",
    userId:
      typeof metadata.userId === "string" ? metadata.userId.slice(0, 100) : "",
    quantity:
      typeof metadata.quantity === "string"
        ? metadata.quantity.slice(0, 10)
        : "1",
  };
};
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (webhookPayload.type === "payment.succeeded") {
      ...
      const { data: existingTicket, error: ticketSelectError } =
      ...
      if (ticketSelectError) {
        console.error("Error checking for existing ticket:", ticketSelectError);
      ...
      if (eventData.current_attendees + quantity > eventData.max_attendees) {
        ...
        return new Response(JSON.stringify({ error: "Event at capacity" }), {
          status: 400,
          headers: { "Content-Type": "application/json", ...corsHeaders },
        });
      }
      ...
      await supabaseClient.from("email_notifications").insert({
        ...
        content: `Your payment of R${(payment.amountInCents / 100).toFixed(
          2
        )} has been confirmed.`,
        template_data: {
