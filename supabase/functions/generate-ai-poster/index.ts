...
function validatePosterRequest(data: unknown): {
  ...
  const eventId = sanitizeString((data as any).eventId, 50);
  if (!eventId || !/^[0-9a-f-]{36}$/.test(eventId))
    errors.push("Invalid eventId");
  const prompt = sanitizeString((data as any).prompt, 500);
  if (!prompt) errors.push("Prompt is required");
  const dimensions = (data as any).dimensions;
  if (
    !dimensions ||
  ...
  const socialPlatform = (data as any).socialPlatform
    ? sanitizeString((data as any).socialPlatform, 30)
    : undefined;
  const style = (data as any).style ? sanitizeString((data as any).style, 30) : undefined;
...
async function generateWithOpenAI(prompt: string, style: string = "vivid"): Promise<string> {
  if (!openAIApiKey) {
...
async function uploadToSupabase(base64Image: string, eventId: string): Promise<string> {
  const supabase = createClient(supabaseUrl, supabaseKey);
  
  // Convert base64 to blob for upload
  const imageBuffer = Uint8Array.from(atob(base64Image), c => c.charCodeAt(0));
  const fileName = `poster-${eventId}-${Date.now()}.png`;
...
Create an eye-catching, professional poster that includes the event information in an attractive layout.`;
    ...
    try {
      ...
      const base64Image = await generateWithOpenAI(enhancedPrompt, style || "vivid");
      imageUrl = await uploadToSupabase(base64Image, eventId);
      ...
      const mockData = await generateMockPoster(enhancedPrompt, dimensions);
...
async function generateMockPoster(
  ...
  console.log("Generating mock poster with prompt:", prompt);
