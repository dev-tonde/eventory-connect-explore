-- Update handle_new_user function to use the new generate_unique_username signature
...
BEGIN
  ...
  -- Generate username with first and last name (firstname.lastname format)
  generated_username := generate_unique_username(COALESCE(user_first_name, 'user'), user_last_name);
  ...
  BEGIN
    ...
    RAISE LOG 'Successfully created profile for user: % with name: % and username: %', new.id, full_name, generated_username;
    
  EXCEPTION 
    WHEN unique_violation THEN
      RAISE LOG 'Username conflict for user %, retrying with different username', new.id;
      -- Retry with timestamp-based username
      generated_username := generate_unique_username(COALESCE(user_first_name, 'user') || '_' || extract(epoch from now())::text, user_last_name);
